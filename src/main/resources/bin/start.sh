...
CLASSPATH=$FULLPATH/$JAR
for jar in $FULLPATH/lib/*.jar; do
...
function doStart(){
	...
	nohup java -Denv=$ENV $JVMARGS -classpath $CLASSPATH -jar $FULLPATH/$JAR --spring.profiles.active=$ENV 1>/dev/null 2>$FULLPATH/error.log &
	for i in $(seq 100)
	do
		if [ -f $FULLPATH/PID.txt ];then
