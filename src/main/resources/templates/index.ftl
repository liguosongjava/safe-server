...
$(function() {
    BJUI.init({
        ...
        loginInfo    : {url:'login_timeout', title:'登录', width:550, height:500}, // 会话超时后弹出登录对话框
        ...
        <div class="container_fluid">
            ...
            <nav class="collapse navbar-collapse" id="bjui-top-collapse">
                <ul class="nav navbar-nav navbar-right">
                    ...
                    <li><a href="#">账号：${loginNo}</a></li>
                    <li><a href="#">用户名：${userName}</a></li>
                    <li><a href="changepwd" data-toggle="dialog" data-id="sys_user_changepass" data-mask="true" data-width="400" data-height="300">修改密码</a></li>
                    <li><a href="#" data-toggle="alertmsg" data-options="{type:'confirm', msg:'你确定要退出登录吗！', okCall:function(){logout();}}"
                           style="font-weight:bold;">&nbsp;<i class="fa fa-power-off"></i> 注销登陆</a>
                    </li>
            ...
            <nav class="collapse navbar-collapse" id="bjui-navbar-collapse">
                <ul class="nav navbar-nav navbar-right" id="bjui-hnav-navbar" >
                    <li class="active hide">
                        <a href="/memu_json" data-toggle="sidenav" data-id-key="targetid">菜单列表</a>
                    </li>
                </ul>
            </nav>
        </div>
...
<script type="text/javascript">
    function logout(){
        $.ajax({url:"logout",success:function(data){
            var json = eval("("+data+")");
